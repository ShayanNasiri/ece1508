#!/bin/bash
#SBATCH --job-name=smollm2-360m-ft-loglik
...
CMD=(python -m recipe_mpr_qa.cli train-slm --config configs/slm_smollm2_360m_finetune_original_highcap_loglikelihood.toml)
...
python "${REPO_ROOT}/slurm/prune_run_checkpoints.py" \
  ...
  --run-id "smollm2-360m-finetune-original-highcap-loglikelihood"
